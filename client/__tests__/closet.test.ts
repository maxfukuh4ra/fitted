...
const makeItem = (overrides: Partial<ClosetItem> = {}): ClosetItem => ({
  ...
  subcategory: 'Hoodie',
  image_url: null,
  created_at: '2026-01-01T00:00:00Z', s
  ...overrides,
});
...
describe('lib/closet-filters', () => {
  describe('formatCategoryLabel', () => {
    ...
    it('title-cases words in subcategory', () => {
      ...
      expect(formatCategoryLabel('sneakers')).toBe('Sneakers');
    });
  });
  ...
  describe('filterItemsByCategory', () => {
    ...
    it('filters by exact subcategory match', () => {
      const filtered = filterItemsByCategory(items, 'Jeans');
      expect(filtered).toHaveLength(1);
      expect(filtered[0].id).toBe('2');
    });

    it('excludes null-subcategory items when a chip is selected', () => {
      const filtered = filterItemsByCategory(items, 'Hoodie');
